...
export async function add_sources_from_vunit(project_manager: Multi_project_manager, prj_name: string){
    ...
    path_list.forEach(async path => {
        await project_manager.add_file_from_vunit(prj_name, project_manager.get_config_global_config(),path);
    });
}